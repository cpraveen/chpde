...
! From the basic clawpack routine step1, rp is called with ql = qr = q.
    ...
    real(kind=8) G(1-mbc:maxmx+mbc)
    logical :: efix
    real(kind=8) :: gammal, gammar, rhsqrtl, rhsqrtr, pl, pr, rhsq2, a1, a2, a3
    real(kind=8) :: rhoim1, pim1, cim1, s0, rho1, rhou1, en1, p1, c1, s1, sfract
    real(kind=8) :: rhoi, pi, ci, s3, rho2, rhou2, en2, p2, c2, s2, df, Gavg
    real(kind=8) :: gamim1, gami, gam1, gam2
    integer :: i, m, mw
...
!     # Compute Roe-averaged quantities:
    ...
    do 20 i=2-mbc,mx+mbc
        ...
        gammal = 1.0d0 + 1.0d0/qr(4,i-1)
        gammar = 1.0d0 + 1.0d0/ql(4,i)
        pl = (gammal-1.0d0)*(qr(3,i-1) - 0.5d0*(qr(2,i-1)**2)/qr(1,i-1))
        pr = (gammar-1.0d0)*(ql(3,i) - 0.5d0*(ql(2,i)**2)/ql(1,i))
        rhsq2 = rhsqrtl + rhsqrtr
        u(i) = (qr(2,i-1)/rhsqrtl + ql(2,i)/rhsqrtr) / rhsq2
        enth(i) = (((qr(3,i-1)+pl)/rhsqrtl &
        + (ql(3,i)+pr)/rhsqrtr)) / rhsq2
        G(i) = (qr(4,i-1)/rhsqrtl + ql(4,i)/rhsqrtr) / rhsq2
        a2 = (1.0d0/G(i))*(enth(i) - 0.5d0*u(i)**2)
        a(i) = dsqrt(a2)
    20 END DO
    ...
    !        # find a1 thru a3, the coefficients of the 3 eigenvectors:
        ...
        a2 = 1.0d0/(G(i)*a(i)**2) * ((enth(i)-u(i)**2)*delta(1) &
             + u(i)*delta(2) - delta(3))
...
!    # if s should change sign.
    ...
    !        ---------------
        ...
        rhoim1 = qr(1,i-1)
        gamim1 = 1.0d0 + 1.0d0/qr(4,i-1)
        pim1 = (gamim1 - 1.0d0)*(qr(3,i-1) - 0.5d0*qr(2,i-1)**2 / rhoim1)
        cim1 = dsqrt(gamim1*pim1/rhoim1)
    ...
    !        # check for fully supersonic case:
        ...
        gam1 = 1.0d0 + 1.0d0/qr(4,i-1)
        p1 = (gam1 - 1.0d0)*(en1 - 0.5d0*rhou1**2/rho1)
        c1 = dsqrt(gam1*p1/rho1)
    ...
    !        ---------------
        ...
        rhoi = ql(1,i)
        gami = 1.0d0 + 1.0d0/ql(4,i)
        pi = (gami - 1.0d0)*(ql(3,i) - 0.5d0*ql(2,i)**2 / rhoi)
        ci = dsqrt(gami*pi/rhoi)
        s3 = ql(2,i)/rhoi + ci     !# u+c in right state  (cell i)
    
        rho2 = ql(1,i) - wave(1,3,i)
        rhou2 = ql(2,i) - wave(2,3,i)
        en2 = ql(3,i) - wave(3,3,i)
        gam2 = 1.0d0 + 1.0d0/ql(4,i)
        p2 = (gam2 - 1.0d0)*(en2 - 0.5d0*rhou2**2/rho2)
        c2 = dsqrt(gam2*p2/rho2)
