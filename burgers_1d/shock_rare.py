...
The initial condition is such that a shock and rarefaction form.
The shock collides with the rarefaction and weakens, eventually
only the shock remains.
"""
...
def setup(nx=1000,use_petsc=0,kernel_language='Fortran',outdir='./_output',
    ...
    x = pyclaw.Dimension(0.0,3.0,nx,name='x')
    domain = pyclaw.Domain(x)
    state = pyclaw.State(domain,solver.num_eqn)

    xc = state.grid.x.centers
    state.q[0,:] =   (xc < 1.0)*(2.0) \
                   + (xc >= 1.0)*(xc <= 2.0)*(-1.0) \
                   + (xc > 2.0)*(0.0)
