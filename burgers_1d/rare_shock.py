...
The initial condition is such that a rarefaction and shock, both moving 
to right.  The rarefaction meets shock and weakens as time goes on.
"""
...
def setup(nx=1000,use_petsc=0,kernel_language='Fortran',outdir='./_output',
    ...
    x = pyclaw.Dimension(-1.0,2.0,nx,name='x')
    domain = pyclaw.Domain(x)
    state = pyclaw.State(domain,solver.num_eqn)

    xc = state.grid.x.centers
    state.q[0,:] =   (xc < 0.0)*(xc > 0.5)*(0.0) + (xc >= 0.0)*(xc <= 0.5)*(0.5)
