...
c     # negate the normal velocity:
      do 136 j = 1-mbc, my+mbc
         do 136 ibc=1,mbc
            alf = aux(1,1,j)
            beta = aux(2,1,j)
            unorm = alf*q(2,ibc,j) + beta*q(3,ibc,j)
            utang = -beta*q(2,ibc,j) + alf*q(3,ibc,j)
            unorm = -unorm
            q(2,1-ibc,j) = alf*unorm - beta*utang
            q(3,1-ibc,j) = beta*unorm + alf*utang
  136    continue
      go to 199
...
c     # negate the normal velocity:
      do 236 j = 1-mbc, my+mbc
         do 236 ibc=1,mbc
            alf = aux(1,mx+1,j)
            beta = aux(2,mx+1,j)
            unorm = alf*q(2,mx+1-ibc,j) + beta*q(3,mx+1-ibc,j)
            utang = -beta*q(2,mx+1-ibc,j) + alf*q(3,mx+1-ibc,j)
            unorm = -unorm
            q(2,mx+ibc,j) = alf*unorm - beta*utang
            q(3,mx+ibc,j) = beta*unorm + alf*utang
  236    continue
      go to 299
